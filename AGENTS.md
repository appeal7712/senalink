...
| 폭 | 공격 | 방어 |
|----|------|------|
| **≤1020** | 1열·인라인 카운터. 카운터 행 그리드: 제목 위 / 초상 아래, 수정·삭제 세로. **이 블록 CSS는 베이스 `.gw-attack-detail` / `.gw-attack-inline-counters`보다 아래에 둘 것** (안 그러면 카운터가 항상 목록 밑으로 감) | (공용 PvP ≤1024 스택과 별도) |
...
- 최근 호스팅 버전대: **v2026.08.27.69** 전후 (푸터 `APP_VERSION` 확인)
